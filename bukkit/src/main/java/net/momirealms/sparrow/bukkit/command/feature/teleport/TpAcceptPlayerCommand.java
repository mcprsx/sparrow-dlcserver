...
public class TpAcceptPlayerCommand extends BukkitCommandFeature<CommandSender> {

    public TpAcceptPlayerCommand(SparrowCommandManager<CommandSender> sparrowCommandManager) {
        super(sparrowCommandManager);
    }

    @Override
    public String getFeatureID() {
        return "tpaccept_player";
    }
        ...
        return builder
                ...
                .handler(commandContext -> {
                    ...
                    if (requester == null || !requester.isOnline()) {
                        handleFeedback(commandContext, MessageConstants.COMMANDS_TELEPORT_TPACCEPT_OFFLINE);
                    ...
                    if (request.isTpHere()) {
                        ...
                        handleFeedback(commandContext, MessageConstants.COMMANDS_TELEPORT_TPACCEPT_SUCCESS_TPHERE, Component.text(requester.getName()));
                        
                        Component notifyMsg = TranslationManager.render(MessageConstants.COMMANDS_TELEPORT_TPACCEPT_NOTIFY.arguments(Component.text(player.getName())).build());
                        requester.sendMessage(notifyMsg);
                    } else {
                        // Tpa: requester teleports to target
                        requester.teleportAsync(player.getLocation());
                        handleFeedback(commandContext, MessageConstants.COMMANDS_TELEPORT_TPACCEPT_SUCCESS_TPA, Component.text(requester.getName()));
